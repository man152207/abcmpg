...
<head>
    ...
    <style>
        ...
        .watermark {
            ...
            z-index: -1;
            opacity: 0.05;
            background-image: url('{{ asset('uploads/tasbirs/watermark.png') }}');
...
<body>
    ...
    <!-- Header Section -->
    <div class="header">
        <img src="{{ asset('uploads/tasbirs/logofoqu.png') }}" alt="MPG Solution Logo">
        <h1>MPG Solution</h1>
    ...
    <div class="content">
        ...
        <h2>Quotation for {{ $quotation->service_details }}</h2>
        ...
        <p><strong>Target Location:</strong> {{ $quotation->target_location }}</p>
        <p><strong>Age Range:</strong> {{ $quotation->age_range }}</p>
        <p><strong>Gender:</strong> {{ ucfirst($quotation->gender) }}</p>

        <h3>4. Campaign Objectives:</h3>
        @if($quotation->service_details == 'Platform Advertising' || $quotation->service_details == 'Lead Generation')
        <p>{{ $quotation->campaign_objectives }}</p>

        <!-- Estimated Results -->
        <h3>5. Estimated Results</h3>
        <p><strong>Estimated Reach:</strong> {{ number_format(($quotation->budget / 160) * 10000) }} people</p>
        <p><strong>Estimated Interactions:</strong> {{ number_format(($quotation->budget / 160) * 500) }} interactions</p>

        <p>
            @if($quotation->campaign_objectives == 'Brand Awareness')
                ...
                <strong>Estimated Results:</strong> Based on your selected budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days, we estimate reaching between {{ number_format(($quotation->budget / 160) * 5000) }} and {{ number_format(($quotation->budget / 160) * 10000) }} users.
            @elseif($quotation->campaign_objectives == 'Reach')
                ...
                <strong>Estimated Results:</strong> With a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days, we estimate reaching {{ number_format(($quotation->budget / 160) * 6000) }} to {{ number_format(($quotation->budget / 160) * 12000) }} people.
            @elseif($quotation->campaign_objectives == 'Traffic')
                ...
                <strong>Estimated Results:</strong> Expect {{ number_format(($quotation->budget / 160) * 300) }} to {{ number_format(($quotation->budget / 160) * 900) }} clicks to your website with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Engagement')
                ...
                <strong>Estimated Results:</strong> Achieve {{ number_format(($quotation->budget / 160) * 1200) }} to {{ number_format(($quotation->budget / 160) * 3000) }} interactions with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'App Installs')
                The objective is to encourage users to download and install your mobile application via targeted ads on Facebook.
                <strong>Estimated Results:</strong> Drive {{ number_format(($quotation->budget / 160) * 100) }} to {{ number_format(($quotation->budget / 160) * 200) }} app installs with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Video Views')
                ...
                <strong>Estimated Results:</strong> Attain {{ number_format(($quotation->budget / 160) * 10000) }} to {{ number_format(($quotation->budget / 160) * 20000) }} video views with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Lead Generation')
                ...
                <strong>Estimated Results:</strong> Capture {{ number_format(($quotation->budget / 160) * 100) }} to {{ number_format(($quotation->budget / 160) * 200) }} leads per week with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Conversions')
                ...
                <strong>Estimated Results:</strong> Generate {{ number_format(($quotation->budget / 160) * 50) }} to {{ number_format(($quotation->budget / 160) * 100) }} conversions per month with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Store Traffic')
                The aim is to drive foot traffic to your physical store locations, using localized targeting to reach nearby customers.
                <strong>Estimated Results:</strong> Expect an increase in store visits by 20% to 30% with a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days.
            @elseif($quotation->campaign_objectives == 'Get More Messages')
                ...
                <strong>Estimated Results:</strong> Expect to receive {{ number_format(($quotation->budget / 160) * 2) }} to {{ number_format(($quotation->budget / 160) * 6) }} additional messages per day, with an increase in response rate and customer satisfaction. Based on a budget of Rs {{ number_format($quotation->budget, 2) }} over {{ $quotation->duration }} days, your ad could reach {{ number_format(($quotation->budget / 160) * 9000) }} to {{ number_format(($quotation->budget / 160) * 26000) }} accounts.
            @endif
        </p>
        @elseif($quotation->service_details == 'Content Strategy & Consultation' || $quotation->service_details == 'Graphic Design' || $quotation->service_details == 'Video Making and Editing')
        <h3>5. Service Objectives</h3>
        <p>Based on your selected service of {{ $quotation->service_details }}, the focus will be on providing tailored strategies and designs that align with your brand's vision and goals.</p>
        <ul>
            @if($quotation->service_details == 'Content Strategy & Consultation')
            ...
            @elseif($quotation->service_details == 'Graphic Design')
            ...
            @elseif($quotation->service_details == 'Video Making and Editing')
    ...
    <div class="signature-section">
        <img src="{{ asset('uploads/tasbirs/Sign.png') }}" alt="Signature" class="sign">
        <p>Man P. Gurung<br><strong>Managing Director</strong></p>
        <p>MPG Solution Private Limited</p>
        <img src="{{ asset('uploads/tasbirs/Stamp.png') }}" alt="Stamp" class="stamp">
    </div>
</div>
    </div>
        ...
        <!-- Footer Section -->
        <div class="footer">
    <p>&copy; 2017-{{ \Carbon\Carbon::now()->year }} MPG Solution, All Rights Reserved.</p>
</div>
