    ...
    public function store(Request $request)
{
    ...
    $customerData['created_by'] = $adminId;               // ✅ यहीं set

    // optional: debug गर्न एक पटक log
    \Log::info('Creating customer', ['admin_id' => $adminId, 'admin_guard' => Auth::guard('admin')->check()]);
    ...
    public function search(Request $request)
{
    ...
    // added by (admin name) बाट खोज्ने
    ...
    {
        $customer = Customer::findOrFail($id);
        $adsQuery = Ad::where('customer', $customer->phone)->orderBy('created_at', 'desc');
        $paginatedAds = $adsQuery->paginate(5);
        $ads = $adsQuery->get();

        $totalUSDAllTime = $adsQuery->sum('USD');
        $totalNPRAllTime = $adsQuery->sum('NRP');
        $totalQuantityAllTime = $adsQuery->sum('Quantity');

        $totalUSDThisMonth = $adsQuery->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])->sum('USD');
        $totalNPRThisMonth = $adsQuery->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])->sum('NRP');
        $totalQuantityThisMonth = $adsQuery->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])->sum('Quantity');

        $totalUSDThisToday = $adsQuery->whereDate('created_at', Carbon::today())->sum('USD');
        $totalNPRThisToday = $adsQuery->whereDate('created_at', Carbon::today())->sum('NRP');
        $totalQuantityThisToday = $adsQuery->whereDate('created_at', Carbon::today())->sum('Quantity');
    ...
    public function toggleRequiresBill(Request $request, $id)
{
    ...
    // admin मात्र बदल्न पाउने – चाहनु भए customer guard पनि मिलाउन सकिन्छ
    if (!auth('admin')->check()) {
...
// --- ADD: lightweight list for dropdown / datalist ---
public function minimal(\Illuminate\Http\Request $request)
{
...
// --- ADD: lookup-by-phone / WhatsApp ---
public function lookupByPhone(\Illuminate\Http\Request $request)
{
...
public function quickSearch(\Illuminate\Http\Request $request)
{
