...
<form method="post" action="<?php echo e(route('admin.daily-logs.update',$log)); ?>">
  ...
  <div class="card p-3">
    <div style="display:flex;gap:12px;flex-wrap:wrap">
      <div>
        <label>Date *</label>
        <input type="date" name="log_date" value="<?php echo e(old('log_date', optional($log->log_date)->toDateString())); ?>" required>
      </div>
      <div>
        <label>Status</label>
        <select name="status">
          <option value="submitted" <?php if(old('status',$log->status)==='submitted'): echo 'selected'; endif; ?>>Submitted</option>
          <option value="draft" <?php if(old('status',$log->status)==='draft'): echo 'selected'; endif; ?>>Draft</option>
          <?php if(auth('admin')->id() === 1): ?>
            <option value="approved" <?php if(old('status',$log->status)==='approved'): echo 'selected'; endif; ?>>Approved</option>
          <?php endif; ?>
        </select>
      </div>
    </div>
  ...
  <div class="card p-3">
    ...
    <textarea name="summary" rows="3"><?php echo e(old('summary', $log->summary)); ?></textarea>
  </div>
  ...
  <div class="mt-3">
    ...
    <a class="btn" href="<?php echo e(route('admin.daily-logs.index')); ?>">Back</a>
  </div>
</form>
...
<script>
  ...
  const prodData = <?php echo json_encode($log->production_array, 15, 512) ?>;
  const recData  = <?php echo json_encode($log->reception_array, 15, 512) ?>;
  const opsData  = <?php echo json_encode($log->operations_array, 15, 512) ?>;
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/mpgcomnp/app.mpg.com.np/resources/views/admin/daily_logs/edit.blade.php ENDPATH**/ ?>