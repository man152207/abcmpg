...
<div class="chat-container flex mx-auto rounded-xl shadow-lg overflow-hidden">
    ...
    <div class="customer-list w-1/4 border-r border-gray-200 flex flex-col" style="background-color: var(--bubble-bg);">
        ...
        <div class="flex-1 overflow-y-auto">
            <?php $__currentLoopData = $customers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="p-4 hover:bg-gray-100 cursor-pointer flex items-center" onclick="selectCustomer('<?php echo e($customer->id); ?>', '<?php echo e($customer->name); ?>')">
                    <div class="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center mr-3">
                        <span class="text-white font-semibold"><?php echo e(substr($customer->name, 0, 1)); ?></span>
                    </div>
                    <div>
                        <p class="font-semibold"><?php echo e($customer->name); ?></p>
                        <p class="text-sm text-gray-500"><?php echo e($customer->phone); ?></p>
                    </div>
                </div>
    ...
    <div class="conversation-pane flex-1 flex flex-col">
        ...
        <div class="chat-messages flex-1 p-4 overflow-y-auto flex flex-col gap-2" id="chatMessagesWrapper">
            ...
            <div id="chatMessages">
                <?php $__currentLoopData = $chats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="chat-message <?php echo e($c->customer_id ? 'ml-auto admin' : 'mr-auto'); ?> relative" data-id="<?php echo e($c->id); ?>" tabindex="0">
                        <div class="message-content">
                            <?php if($c->customer_id && $c->customer): ?>
                                <p class="text-sm font-bold <?php echo e($c->customer_id ? 'text-white' : 'text-gray-600'); ?>">For: <?php echo e($c->customer->name); ?> | <?php echo e($c->admin ? $c->admin->name : 'Unknown Admin'); ?></p>
                            <?php else: ?>
                                <p class="text-sm font-bold text-gray-600"><?php echo e($c->admin ? $c->admin->name : 'Unknown Admin'); ?></p>
                            <?php endif; ?>
                            <?php if($c->message): ?>
                                <p class="text-base whitespace-pre-wrap"><?php echo e($c->message); ?></p>
                            <?php endif; ?>
                            <?php if($c->image_paths): ?>
                                <?php $__currentLoopData = $c->image_paths; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image_path): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <img src="<?php echo e(asset('storage/' . $image_path)); ?>" class="message-image cursor-pointer" alt="Chat Image" loading="lazy" onclick="openImageModal('<?php echo e(asset('storage/' . $image_path)); ?>')">
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php endif; ?>
                            <p class="text-sm text-gray-500 mt-1" style="justify-self: end;">
                                <?php echo e($c->created_at->format('M d, Y h:i A')); ?>

                            </p>
                        </div>
                        <!-- Context Menu -->
                        <div class="context-menu hidden" id="contextMenu-<?php echo e($c->id); ?>">
                            <button type="button" class="edit-message" data-id="<?php echo e($c->id); ?>">Edit</button>
                            <button type="button" class="delete-message" data-id="<?php echo e($c->id); ?>">Delete</button>
                            <button type="button" onclick="copyToClipboard('<?php echo e(addslashes(strip_tags($c->message ?? ''))); ?>')">Copy</button>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        ...
        <div class="chat-input-section px-3 py-2 border-t" style="border-color: var(--border-color); background-color: var(--bubble-bg);">
            <form id="chatForm" method="POST" enctype="multipart/form-data" class="flex flex-col gap-2" action="<?php echo e(route('admin.internal_chat.store')); ?>">
                <?php echo csrf_field(); ?>
...
<script>
    const CURRENT_ADMIN_ID = <?php echo e(auth('admin')->id()); ?>;
    ...
    function appendMessageToChat(response, message, customerId, timestamp) {
        ...
        imagePaths.forEach(path => {
            const fullPath = `<?php echo e(asset('storage')); ?>/${path}`;
        ...
        Swal.fire({
            ...
            didOpen: () => {
                $('.reaction-btn').on('click', function() {
                    ...
                    $.ajax({
                        url: '<?php echo e(route("admin.internal_chat.addReaction")); ?>',
                        type: 'POST',
                        data: { chat_id: chatId, emoji: emoji, _token: '<?php echo e(csrf_token()); ?>' },
    ...
    $(document).on('click', '.edit-message', function() {
        ...
        $.ajax({
            url: `<?php echo e(route('admin.internal_chat.show', '')); ?>/${chatId}`,
            type: 'GET',
            headers: { 'Accept': 'application/json' },
            success: function(chat) {
                const form = $('#editChatForm');
                form.attr('action', `<?php echo e(route('admin.internal_chat.update', '')); ?>/${chat.id}`);
    ...
    $(document).on('click', '.delete-message', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `<?php echo e(route("admin.internal_chat.delete", "")); ?>/${chatId}`,
                    type: 'POST',
    ...
    $('#searchInput').on('input', debounce(function() {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.internal_chat.search")); ?>',
            data: { q: query },
            success: function(response) {
    ...
    function loadMoreMessages() {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.internal_chat.loadMore")); ?>',
            data: { page: page },
            success: function(response) {
...
<?php echo $__env->make('admin.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/mpgcomnp/app.mpg.com.np/resources/views/admin/chat/internal_chat.blade.php ENDPATH**/ ?>